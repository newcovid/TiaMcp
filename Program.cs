        ...
        internal static int Dispatch(string[] args)
        {
            ...
            try
            {
                switch (cmd)
                {
                    ...
                    case "hmi-read-template-layout":
                        if (args.Length < 2) { Console.WriteLine("用法: hmi-read-template-layout <模板名>"); return 1; }
                        return HmiReads.HmiReadTemplateLayout(args[1]);

                    case "hmi-find-unused-tags":
                        return HmiReads.HmiFindUnusedTags();

                    case "hmi-tag-usage":
                        if (args.Length < 2) { Console.WriteLine("用法: hmi-tag-usage <变量名>"); return 1; }
                        return HmiReads.HmiTagUsage(args[1]);
                    ...
                    default:
                        ...
                        Console.WriteLine("  HMI: hmi-list | hmi-read-tags [--table 名][--filter 子串] | hmi-read-screens | hmi-read-screen <名> | hmi-read-screen-layout <名> | hmi-read-templates | hmi-read-template-layout <名> | hmi-read-connections | hmi-export-all <目录> | hmi-find-unused-tags | hmi-tag-usage <变量>");
