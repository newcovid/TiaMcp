...
- **所属模板（母版）提示（2026-06-05 新增）**：输出顶部标注本画面「所属模板(母版)」（取自画面根 `<LinkList><Template><Name>`，如 `0010 车体状态` → `全局画面`，**不是** `模板_1`）。**画面控件叠加在模板之上显示**——模板提供页眉/页脚/导航/永久区等公共元素，这些**不在本画面的控件清单里**。故若画面有模板，输出会提醒：**再调 `hmi-read-template-layout "<模板名>"` 读模板布局，与本画面布局综合考虑**，才能完整还原视觉呈现。未用模板的画面显示「无」；读取模板本身时不显示此行。
...
- **输出示例**（实测 `0010 车体状态`，已与 `hmi-export-screen` 导出 XML 交叉验证一致）：
  ...
  ==== HMI_1 · 画面布局 0010 车体状态 ====
    ...
    背景色: 232, 237, 243
    所属模板(母版): 全局画面
    ⚠ 本画面的控件叠加在模板「全局画面」之上显示——模板提供页眉/页脚/导航/永久区等公共元素,不在下方控件清单里。
      要完整理解本画面的视觉呈现,请同时读取模板布局: hmi-read-template-layout "全局画面"，再与本画面布局综合考虑。
