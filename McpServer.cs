...
namespace TiaMcp
{
    ...
    internal static class McpServer
    {
        ...
        private static readonly ToolDef[] Tools = new[]
        {
            ...
            new ToolDef{ Name="hmi-read-screen-layout", Desc="✅ 画面完整视觉布局+动态行为:控件位置(X,Y)/大小(W×H)/颜色(背景/前景/边框)/字体/文本/变量绑定/圆角/透明度/重叠检测 + 动画(范围外观变色/闪烁/可见性控制) + 事件(点击跳转/按下动作/画面级事件)。用于理解画面结构、规划UI修改、分析动态效果。优先使用此命令而非 hmi-read-screen。注意:①动画统计含子控件(Group/ScreenLayer的动画数=自身+所有子控件之和,同一动画在父子各出现一次,勿重复计数);②控件计数含嵌套Property(ProcessValue,即IOField/Bar的子属性);③Group无自身位置/大小,取第一个子控件的值", Req=new[]{"screenName"},
                P=new Dictionary<string,string>{ ["screenName"]="画面名" } },
            new ToolDef{ Name="hmi-read-templates", Desc="模板画面(母版)树(仅名称,无控件详情)。强烈建议改用 hmi-read-template-layout 获取完整视觉布局+动态行为" },
            new ToolDef{ Name="hmi-read-template-layout", Desc="✅ 模板(母版)完整视觉布局+动态行为:控件位置/大小/颜色/字体等视觉属性 + 动画(范围外观变色/闪烁/可见性控制) + 事件(点击/按下等)。与 hmi-read-screen-layout 相同的信息,但针对模板画面。优先使用此命令而非 hmi-read-templates。注意:①动画统计含子控件(Group/ScreenLayer的动画数=自身+所有子控件之和,同一动画在父子各出现一次,勿重复计数);②控件计数含嵌套Property(ProcessValue,即IOField/Bar的子属性);③Group无自身位置/大小,取第一个子控件的值", Req=new[]{"templateName"},
